package com.pisien.edu.pro.pro07IOPackage02.example07;
...
public class DataInOutputStreamEx {
	...
	public static void main(String[] args) throws Exception {
		//파일 객체 생성
		File file = new File("C:/Temp/primitiveData.dat");
		//주 스트림 생성
		FileOutputStream fOutputStream = new FileOutputStream(file);
		System.out.println("파일의 크기  : " + file.length()); //0바이트

		//보조스트림 연결
		//기본타입(int, long, double, float, String..등)출력을 하기 위한 스트림이 바로
		//DataOutputStream이다.
		...
		dOutputStream.writeUTF("김연아");   //11바이트
		System.out.println("김연아 저장 후-파일의 크기  : " + file.length() + "byte");
		...
		//프로그램으로 읽어들일 때에는 저장할 때의 순서와 반드시 동일하게 읽어와야 한다.(중요함)
		for(int i=0; i<2; i++) {
